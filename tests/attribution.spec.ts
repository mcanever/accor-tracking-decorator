    ...
    describe('detectReferrer', () => {
        const validURLs:any = {
            ...
            SEO: {
                ...
                WOLFRAMALPHA: 'https://www.wolframalpha.com/input/?i=grand+mercure', //NO
            ...
            if (validURLs.hasOwnProperty(category)) {
                ...
                for (let name in section) {
                    if (section.hasOwnProperty(name)) {
                        const URLtoTtest = section[name];
                        const URLtoTestReduced = URLtoTtest.slice(0,70) + '...';
                        const sourceid = category + '_' + name ;
                        console.log ('Should detect ' + sourceid + ' if the referrer is ' + URLtoTestReduced);
                        it('Should detect ' + sourceid + ' if the referrer is "' + URLtoTestReduced + '"', () => {
                            const res = Attribution.detectReferrer(URLtoTtest);
                            if (URLtoTtest == '') {
